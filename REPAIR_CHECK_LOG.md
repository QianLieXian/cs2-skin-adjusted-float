...
## 2026-04-01（库存 0 件问题复修 + 一键刷新库存 + 后端日志导出）

### 问题现象
- 你已经登录成功，但前端显示“已读取库存材料 0 件”。
- 即使填写了 Steam Web API Key，结果仍是 0，容易误判成“Key 无效”或“账号真没库存”。
- `npm run start -- --verbose` 只能看实时控制台，信息不完整且不方便打包给我复查。

### 本轮修复
1. **新增“刷新库存”按钮（前端）**
   - 登录后可手动触发 `/api/inventory` 再拉一次，不用刷新整页。
   - 状态区会显示“已刷新库存：X 件”或明确错误原因。

2. **新增“导出后端日志”按钮（前端 + 后端）**
   - 后端新增内存日志缓冲区，自动记录 `log/info/warn/error/debug`。
   - 新增接口 `/api/logs/export`，可下载完整日志文本，便于你发我排障。

3. **修复“已登录但库存 0 件”的核心链路**
   - `/api/inventory` 改为“双通道读取”：
     - 优先：`STEAM_WEB_API`（有 Key 时）。
     - 回退：`steamcommunity.com/inventory/{steamId}/730/2`（无 Key 或 Key 路径返回 0 时自动启用）。
   - 返回 `source` 会明确标识：
     - `auth`（Key 路径命中）
     - `auth_fallback_community`（Key 路径 0 件后自动回退）
     - `community`（未提供 Key，直接 community 路径）

### 结果
- 你现在即使 Key 路径拿不到物品，也会自动尝试 community 库存接口，不再轻易显示 0。
- 可以一键刷新库存验证是否生效。
- 可以一键导出完整后端日志给我，我后续定位会快很多，不用你手抄控制台。
