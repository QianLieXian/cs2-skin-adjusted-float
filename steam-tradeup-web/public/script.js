...
function bestCandidates(targetFloat, skins, inventory, topN = 15) {
  ...
  const avgInputFloat = inventory.reduce((s, x) => s + x.floatValue, 0) / inventory.length;
  return skins
    .map((skin) => {
...
function renderResult(rows, targetFloat) {
  if (!rows.length) {
    ui.result.innerHTML = '<p class="warn">未检测到可用库存材料，请先登录读取库存或加载示例库存。</p>';
    return;
  }
  ...
  ui.result.innerHTML = `
    ...
    <p>当前库存用于炼金的 10 件材料平均磨损：<strong>${fmt16(rows[0].avgInputFloat)}</strong></p>
...
function loadDemoInventory() {
  inventoryItems = [
    ...
    0.1177777777777777,
    0.101010101010101
  ].map((f, idx) => ({ id: `demo-${idx}`, floatValue: f }));
